...
def kpi_metcalfe(bc):
    ...
    seff = effective_supply(dates, supply)
    ...
    return {'value': round(float(met[-1]), 2), 'k': float(np.exp(k)), 'sigma_pts': round(sig, 1), 'r2_window': round(float(r2), 3), 'premium_pct_close': round(float(prem[-1]), 1),
